...
// Calcula la fecha máxima permitida basada en el periodo seleccionado
const fechaMaximaPermitida = computed(() => {
  if (!periodoSeleccionado.value) return new Date().toISOString().split('T')[0]; // Si no hay periodo, hoy.

  // Buscamos el texto del periodo (ej: "2025-08")
  const periodoObj = periodos.value.find(p => p.id_periodo === periodoSeleccionado.value);
  
  if (periodoObj) {
    // Calculamos el último día de ese mes
    const [anio, mes] = periodoObj.periodo.split('-');
    const ultimoDia = new Date(anio, mes, 0).getDate(); // Día 0 del siguiente mes es el último de este
